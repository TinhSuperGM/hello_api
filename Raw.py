...
class FightSession:
    ...
    async def render_step(self, message: discord.Message, logs_from_action: List[str]) -> None:
        for line in logs_from_action:
            self.add_log(line)
        try:
    ...
    async def play_round(self, message: discord.Message) -> None:
        ...
        for side in (first_side, second_side):
            ...
            await self.render_step(message, action_logs)
