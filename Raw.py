...
import random
import time
from copy import deepcopyfrom __future__ import annotations
...
class FightSession:
    ...
    def stamp(self, text: str) -> str:
        return f'{{{int(self.elapsed_seconds)}s}} {text}'

    async def animate_hp_change(
        self,
        message: discord.Message,
        target: dict,
        start_hp: int,
        end_hp: int,
        duration: float = ACTION_DELAY,
        steps: int = 8,
    ) -> None:
        if start_hp == end_hp:
        ...
        steps = max(3, min(steps, abs(end_hp - start_hp) or 3))
    ...
    def attack(self, attacker: dict, defender: dict) -> str:
        ...
        if is_crit and random.choice(['damage', 'heal']) == 'heal':
            ...
            if is_combo:
                return self.stamp(f"❤️‍🔥{attacker['name']} đã hồi máu combo và nhận được {actual_heal}HP.")
            return self.stamp(f"❤️‍🩹 {attacker['name']} đã hồi máu và nhận được {actual_heal}HP.")
        ...
        if defender['hp'] <= 0:
            ...
            return self.stamp(
                f"🩸 {attacker['name']} đánh {defender['name']} gây {damage} dame. {defender['name']} đã bị hạ gục. Love giảm từ {old_love} còn {new_love}."
            )
