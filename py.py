...
class BidView(discord.ui.View):
    ...
    @discord.ui.button(
        label="💰 Đặt gold đấu giá",
        style=discord.ButtonStyle.green,
        custom_id="bid_button"
    )
...
# ===== COMMAND =====
@app_commands.command(name="dau-gia", description="Đăng bán waifu")
async def dau_gia(interaction: discord.Interaction, waifu_id: str, min_price: int, step: int):
    ...
    while not bot.is_closed():
        ...
        for auction_id, auction in auctions.items():
            ...
            if now < auction.get("end_time", 0):
                # Nếu chưa hết hạn, chỉ update embed
                for msg_info in auction.get("messages", []):
                ...
                for msg_info in auction.get("messages", []):
                    ...
                    async def end_message(mi=msg_info, ch=channel):
                        try:
                            if highest:
                                # Chuyển waifu về người thắng
                                inv.setdefault(highest, {}).setdefault("waifus", {})
                                inv[highest]["waifus"][waifu_id] = 1
                                # Người bán nhận tiền đã đấu giá
                                if final_bid > 0:
                                    data_user.add_gold(seller, final_bid)

                                desc = (
                                    f"<@{highest}> đã chiến thắng cuộc đấu giá này và dành được **{waifu_id}** "
                                    f"về tay mình với số tiền cuối cùng là {final_bid}.\n"
                                    f"Số tiền đấu giá này đã được chuyển về cho chủ cũ là <@{seller}>."
                                )
                            else:
                                # Không ai đấu giá -> trả waifu cho seller
                                inv.setdefault(seller, {}).setdefault("waifus", {})
                                inv[seller]["waifus"][waifu_id] = 1
                                desc = (
                                    f"Không có ai đặt giá cho **{waifu_id}**, waifu đã được trả lại cho <@{seller}>."
                                )
        ...
        if ended_auctions:
            # Xóa các auction đã kết thúc khỏi file
...
# ===== SETUP =====
async def setup(bot):
    bot.tree.add_command(dau_gia)

    # 🔥 Khôi phục button sau restart
    auctions = load_json(AUCTION_FILE)
